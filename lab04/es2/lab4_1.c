...
void sortmatricola(studenti *studente, int N);
...
void selettore(studenti *studente, int N)
{
	...
	switch (selezione)
	{
	case 1: sortmatricola(studente, N);
		mark = 1;
		break;
	...
	for (i = 0;i < N;i++)
	{
		for (a = i + 1;a < N;a++)
		{
			...
			if (studente[a].cognome[0] == studente[i].cognome[0])
			{
				...
				{
					flag = sortalfa2(studente[a].nome, studente[i].nome, a, l);
				}
				else flag = sortalfa2(studente[a].cognome, studente[i].cognome, a, l);
			}
		}
		if (flag != -1)
		{
